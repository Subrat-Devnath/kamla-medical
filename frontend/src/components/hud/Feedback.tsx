...
function ErrorBanner({
  ...
  return (
    <div
      role="alert"
      className="flex items-start gap-3 rounded-2xl border border-rose-500/25 bg-rose-500/10 p-3.5 text-sm text-rose-200 sm:p-4"
    >
      ...
      {onDismiss && (
        <button
          ...
          className="-m-1 shrink-0 rounded-lg p-1 text-rose-300/70 transition-colors hover:text-rose-200"
        >
          <X size={15} />
        </button>
      )}
...
function SuccessBanner({ message }: { message: string }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-3.5 text-sm text-emerald-200 sm:p-4">
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="relative h-1 w-full max-w-xs overflow-hidden rounded-full bg-white/10">
        <div className="absolute inset-y-0 w-1/3 animate-shimmer rounded-full bg-gradient-to-r from-transparent via-cyan-400 to-transparent" />
      </div>
      <span className="hud-label shrink-0 text-cyan-300">{label}</span>
    </div>
  );
}
...
function EmptyState({
  ...
  return (
    <div className="hud-surface flex flex-col items-center gap-3 px-6 py-12 text-center sm:py-16">
      <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.04] text-slate-500 ring-1 ring-white/10">
        <Icon size={24} />
      </span>

      <h3 className="text-base font-bold text-slate-200 sm:text-lg">{title}</h3>
